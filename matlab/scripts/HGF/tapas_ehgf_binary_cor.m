...
for k = 2:1:n
    ...
    if not(ismember(k-1, r.ign))
        ...
        % Prediction
        x = ka(1) *muhat(k,2);
        muhat(k,1) = tapas_sgm(x, 1);
        
        if muhat(k,1) >= 0.95;
            muhat(k,1)=0.95;
        elseif muhat(k,1) <= 0.05;
            muhat(k,1)=0.05;
        end
        ...
        %pihat(k,2) = 1/(1/pi(k-1,2) +exp(ka(2) *1 +om(2)));
        %pihat(k,2) = 1/(1/2 +exp(ka(2) *mu(k-1,3) +om(2)));
        %pihat(k,2)=2;
        ...
        % update
        vv(k,l) = t(k) *exp(ka(l-1) *mu(k,l) +om(l-1));
        pimhat(k,l) = 1/(1/pi(k-1,l-1) +vv(k,l)); 
        %pimhat(k,l) = 1/(1/2 +vv(k,l)); 
        %pimhat = 1; det virker ikke at sætte pimhat = 1
        ww(k,l) = vv(k,l) *pimhat(k,l);
        rr(k,l) = (vv(k,l) -1/pi(k-1,l-1)) *pimhat(k,l); %pi(k-1) = 0 i l-1, derfor står der 0*pimhat=0
        dd(k,l) = (1/pi(k,l-1) +(mu(k,l-1) -muhat(k,l-1))^2) *pimhat(k,l) -1;
      ...
      % Updates
       ...
       %end
        ...
        pi(k,l) = pihat(k,l) +max(0, 1/2 *ka(l-1)^2 *ww(k,l)*(ww(k,l) +rr(k,l)*dd(k,l))); %sidste led er bare ww*ww = (vv*pimhat)^2
        % Volatility prediction error
