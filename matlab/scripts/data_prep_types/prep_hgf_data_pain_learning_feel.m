function prep_hgf_data_pain_learning_feel(nSubjects, removers,mainScriptPath)
...
dataFolder      = fullfile(fullfile(mainScriptPath, 'matlab data'));              % <--- EDIT THE DATA FOLDER HERE
% subDirectories  = dir(dataFolder);
FileList  = dir(dataFolder);
% searchString    = 'TPL_v*-1_0';                                                                     % <--- AND THE FILE NAME
dataFolderOut   = fullfile(fullfile(mainScriptPath));                              % <--- EDIT OUTPUT FOLDER

tplSequenceFolder = fullfile(fullfile(mainScriptPath, 'painLearning_ver4','tasks','03_TPL'));
...
outcomes1 = outcomes;
procold = nan(length(outcomes1), 1);
        for i = 1:306
         if outcomes1(i) == 2;
             procold(i) = str2double(confidence_ratings(i,1))/(str2double(confidence_ratings(i,2))+str2double(confidence_ratings(i,1)));
           end
        end
...
%         end
        ...
        inputs(cond1) = 0;
        inputs(cond2) = 1;
        inputs(cues == 1 & outcomes1 == 2) = procold(cues == 1 & outcomes1 == 2); %TGI
        inputs(cues == 0 & outcomes1 == 2) = 1-procold(cues == 0 & outcomes1 == 2); %TGI
...
%         end
        ...
        y_conf (:,:,subLoop-2-a) = str2double(confidence_ratings);
        y_conf_rts (:,:,subLoop-2-a)= str2double(confidence_rts);
        stim1 = [stim1, stim];
...
thisSub         = 'multsub_TGI';
DataFileName    = ['HGFwksh_TPL_feel', thisSub,'.mat'];
save(strcat(dataFolderOut, filesep, DataFileName),  'u', 'u_cues','u_densProb','y', 'y_rts', 'y_conf', 'y_conf_rts', 'stim1', 'stormdb1','y_pred','predacc1','v');
