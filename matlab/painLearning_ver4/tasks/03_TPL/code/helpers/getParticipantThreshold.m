...
noThreshFlag = 0;
dataFolder = vars.OutputFolder;       % ####on aux: NiiaTasks/data
...
if strcmp(vars.subIDstring, '9999')   % if we're testing, set a default thresh
    ...
    if length(files) > 1        % if there are more than one files, ask experimenter to pick which one
        disp('More than one file found for this participant. Using most recent threshold file... ');
    ...
    % Change extreme PMFstims to clear A and H faces of 0 & 200
